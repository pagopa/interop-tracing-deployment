...
if [[ -n $microservice ]]; then
  target=$microservice
  targetValues="values-microservice"
else
  target=$job
  targetValues="values-cronjob"
  prefix="JOB_"
fi
...
envsubst < "$CONTAINER_IMAGES_FOLDER"/$targetValues.yaml > "$CONTAINER_IMAGES_FOLDER"/$targetValues.compiled.yaml